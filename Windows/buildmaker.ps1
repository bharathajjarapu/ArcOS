# Tinyll Builder with Custom XML
...
{
    Write-Host "Restarting ArcOS image creator as admin in a new window, you can close this one."
...
Start-Transcript -Path "$PSScriptRoot\ArcOS.log" 

$Host.UI.RawUI.WindowTitle = "ArcOS image creator"
Clear-Host
Write-Host "Welcome to the ArcOS Image Creator!"

$hostArchitecture = $Env:PROCESSOR_ARCHITECTURE
New-Item -ItemType Directory -Force -Path "$ScratchDisk\ArcOS\sources" >null
...
if ((Test-Path "$DriveLetter\sources\boot.wim") -eq $false -or (Test-Path "$DriveLetter\sources\install.wim") -eq $false) {
    if ((Test-Path "$DriveLetter\sources\install.esd") -eq $true) {
        ...
        & 'DISM' /Export-Image /SourceImageFile:"$DriveLetter\sources\install.esd" /SourceIndex:$index /DestinationImageFile:"$ScratchDisk\ArcOS\sources\install.wim" /Compress:max /CheckIntegrity
    } else {
...
Copy-Item -Path "$DriveLetter\*" -Destination "$ScratchDisk\ArcOS" -Recurse -Force > null
Set-ItemProperty -Path "$ScratchDisk\ArcOS\sources\install.esd" -Name IsReadOnly -Value $false > $null 2>&1
Remove-Item "$ScratchDisk\ArcOS\sources\install.esd" > $null 2>&1
Write-Host "Copy complete!"
Start-Sleep -Seconds 2
Clear-Host
Write-Host "Getting image information:"
&  'dism' '/English' "/Get-WimInfo" "/wimfile:$ScratchDisk\ArcOS\sources\install.wim"
$index = Read-Host "Please enter the image index"
Write-Host "Mounting Windows image. This may take a while."
$wimFilePath = "$($env:SystemDrive)\ArcOS\sources\install.wim" 
...
& dism /English "/mount-image" "/imagefile:$($env:SystemDrive)\ArcOS\sources\install.wim" "/index:$index" "/mountdir:$($env:SystemDrive)\scratchdir"
...
$imageInfo = & 'dism' '/English' '/Get-WimInfo' "/wimFile:$($env:SystemDrive)\ArcOS\sources\install.wim" "/index:$index"
...
## This function allows PowerShell to take ownership of the Scheduled Tasks registry key from TrustedInstaller.
...
& 'dism' '/English' '/Export-Image' "/SourceImageFile:$ScratchDisk\ArcOS\sources\install.wim" "/SourceIndex:$index" "/DestinationImageFile:$ScratchDisk\ArcOS\sources\install2.wim" '/compress:recovery'
Remove-Item -Path "$ScratchDisk\ArcOS\sources\install.wim" -Force >null
Rename-Item -Path "$ScratchDisk\ArcOS\sources\install2.wim" -NewName "install.wim" >null
Write-Host "Windows image completed. Continuing with boot.wim."
Start-Sleep -Seconds 2
Clear-Host
Write-Host "Mounting boot image:"
$wimFilePath = "$($env:SystemDrive)\ArcOS\sources\boot.wim" 
& takeown "/F" $wimFilePath >null
& icacls $wimFilePath "/grant" "$($adminGroup.Value):(F)"
Set-ItemProperty -Path $wimFilePath -Name IsReadOnly -Value $false
& 'dism' '/English' '/mount-image' "/imagefile:$ScratchDisk\ArcOS\sources\boot.wim" '/index:2' "/mountdir:$ScratchDisk\scratchdir"
Write-Host "Loading registry..."
...
& 'dism' '/English' '/unmount-image' "/mountdir:$ScratchDisk\scratchdir" '/commit'
Clear-Host
Write-Host "The ArcOS image is now completed. Proceeding with the making of the ISO..."
Write-Host "Copying unattended file for bypassing MS account on OOBE..."
Copy-Item -Path "$PSScriptRoot\autounattend.xml" -Destination "$ScratchDisk\ArcOS\autounattend.xml" -Force >null
...
& "$OSCDIMG" '-m' '-o' '-u2' '-udfver102' "-bootdata:2#p0,e,b$ScratchDisk\ArcOS\boot\etfsboot.com#pEF,e,b$ScratchDisk\ArcOS\efi\microsoft\boot\efisys.bin" "$ScratchDisk\ArcOS" "$PSScriptRoot\ArcOS.iso"

# Finishing up
Write-Host "Creation completed! Press any key to exit the script..."
Read-Host "Press Enter to continue"
Write-Host "Performing Cleanup..."
Remove-Item -Path "$ScratchDisk\ArcOS" -Recurse -Force >null
